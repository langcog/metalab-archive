...
# - Survey of status quo --> what we're doing OK and what we're doing wrong
#     - What do we report or neglect --> demographic & outcome variables, let alone ES!
#      - power based on original versus median --> yikes
#      - power to guide sample size: fields that are doing OK versus not
#      - fields that are not doing OK: could it be the fact that no clear prediction? (i.e. both novelty and familiarity can be expected)
#      - method has a huge effect on ES and probably some are more stable i.e. lower heterogeneity than others
#      - Fuss out X ES (controling for method) --> :)
# 
# - Recommendations
#     - report crucial stuff -- e.g. m SD and learn to understand ES (here para on APA)
#     - use MAs: Who do we look at (citation but also study planning) --> wrong!
#     - make informed decisions, e.g. on method -- take into account average ES, heterogeneity, data exclusion/loss, clarity in response prediction/interpretation (dir of effects)
#     - dangers of small sample sizes: not just underpowered, also overestimates of actual effects
#     - do MAs (para on MAs & ES)
#     - avoid burying your results
#     - we have not checked: reproducibility, sharing data & scripts, documenting, etc. but suspect it's not great - recommend sites
...
#source("../../Desktop/Experiments/MetaLab/Results/metalab/dashboard/global.R", chdir = TRUE) # Page source
library(metafor)
library(dplyr)
library(tidyr)
library(ggplot2)
...
method_exclude_data = all_data %>%
  ...
  mutate(percent_dropout = dropout / (n+dropout)) %>%
  group_by(method) %>%
  mutate(number = n()) %>%
  ungroup() %>%
  filter(number >10) %>%
  mutate(method = factor(method)) %>%
  select(percent_dropout, dropout, dataset, ageC, n, method)
