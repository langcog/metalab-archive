...
the mappings of words to meanings, to name only a few. Moreover, acquiring a language requires not
...
separate research programs. This research shows that, while the onset of these skills can often be found in the first year of life, most follow a protracted pattern of development, with important changes occuring in the second year and beyond. As a result, it would behoove the research community to take into account the development of the whole system at once. 
%However, if it is indeed the case that linguistic skills are interdependent, this both casts some doubt on results based on the isolationist method and suggests that we will not be able to understand one skill without a more precise understanding 
%ac I'm tempted to rewrite this first para to say:
...
The pattern just mentioned (early onset followed by protracted development)
is actually a verbal description that glosses over the fact that such panoramas
are based on limited, noisy experimental findings. In actual fact, there is often one study finding an effect, but another failing to do so.
 These contradictions are sometimes solved verbally, discounting one or the other finding depending on the viewer's expertise (and potentially their theoretical penchant). However, there is an alternative, which is to combine across results in a more systematic and principled fashion.

%What is needed then is a method for determining the degree to which a particular finding provides evidence for a theory.
%ac I think this is a second order. The first step is metalab, a quantitative, rather than verbal, description of the field of language acquisition. The second step involves fitting specific models that are theoretically driven, and then assessing to what extent the whole set of results provides evidence for one versus another theory. So perhaps this should be introduced as a third challenge.
%ac in other words, you'd be opposing metalab to verbal overviews and theories


We suggest a solution to both of these challenges---building integrative
whole-system views  and evaluating evidential strength in a field of scientific research---is to reframe experimental
...
negative, or differences between initial and replication studies, such that
...
there is some variability in true effect size from study to study, meta-analytic methods can provide more veridical description
...
variability around this point estimate. 
...
%ac and here add a third, that once everything is in the same language, you can also see whether specified models fit the data or not, again using MA = quantitative evaluation of theories
...
because testing a theory requires a precise measurement of changes in
...
of the empirical phenomena.
 ...
 In the current work, this common language allows us to
...
gaze following, and many others.%ac still not convinced you can look at relationships ...



Although these tools are broadly applicable to
psychological literatures, language acquisition may be a particularly informative case study. One reason is that language acquisition may
...
phenomena and finally offer an example of quantitative evaluation of theories.

\section{Method}\label{method}

We analyzed 12 different phenomena in language acquisition. To a certain extent, these
 phenomena were selected opportunistically, either because of
...
interactive nature of language acquisition (Table 1).%ac I'm not sure that the data can shed light on interaction. I think it can show development occurring at the same time but this dev could be parallel or interactive.
...
development.%ac I've a really hard time with this - linguist's bias I guess. Induction is fine, but it sounds like double-dipping to do induction from a whole field. That's why I'm suggesting above that this first step is described as providing an unbiased overview of the literature (quantitative, systematic - unlike what people do in their verbal reviews), rather than theory *building* per se. BUT I'm guessing that the proponents of PRIMIR would disagree with me ;)
 We also find evidence for some bias, as well as evidence
...
% bias (CITE). 
Importantly, if experimenters are exercising analytical
flexibility through practices like selective reporting and p-hacking, then the number and
...
literature shows some evidence for bias, but overall, it is quite robust.


\subsection{Quantitative, systematic overview}\label{meta-analytic-effect-size}%ac my intuition is that bias should go first in the way the story is being told. You want to first establish whether the data can be trusted, and then you'll read a story off the data. What do you think?
...
%\subsection{Fail-safe-N}\label{fail-safe-n}%ac I'd remove this whole section, or perhaps move it to SI. Fail-safe-n is not a very intuitive description of *bias* - as you say below; so let's cut to the chase and do funnels + p-curving
...
describes a real effect. We can thus proceed to use these data to evaluate
predictions from competing developmental theories.

\section{Quantitative evaluation of theories}\label{theoretical-development}
...
between different skills.%ac hmmm I buy that you can look at relative timing, but I'm not sure about using the word *relationship*. In fact, your second proposed model ALSO is now introduced as a serial model, but top-down instead of bottom up. I see two solutions, one saying that you'll just look at relative timing; another to relax your definition of the second model to accommodate both parallel & interactive, and then state that -regardless of this dimension- the key way in which the two theories differ is in the timing of higher versus lower
...
strong evidence for interactivity%ac I protest!
---children begin learning even
...
between a larger space of meta-theories and, ultimately, refine our way towards a single
...
are acquired interactively rather than in a strictly bottom-up fashion.%ac I protest!
...
be true to some extent, we find that method does not have a large impact%ac hmmm that's because method is confounded with age, no?
...
In this paper, we focused on seldom-discussed theoretical motivations for building meta-analysis. Naturally, there are many other
practical reasons for conducting a quantitative synthesis. For example, when planning
...
sample size. These and other advantages, illustrated with the same
database used here, are explained in "Educational tool paper".
...
\begin{itemize}
\item
  Educational tool - presage/link to other paper%ac I think that with the para I moved from the discussion, it's enough
\item
  Contributions, CAMAS %ac vote for no
\item
  Other domains -- language acquisition as a case study %ac yes, that would be great!
\end{itemize}
