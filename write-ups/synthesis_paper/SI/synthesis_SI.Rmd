...
This document was created from an [R Markdown file](https://github.com/mllewis/xtSamp/blob/master/analysis/analysis.Rmd).
Data from the paper can be interactively explore on the [Metalab website](http://metalab.stanford.edu/). The manuscript itself was also produced from an  [R markdown file](https://github.com/langcog/metalab/blob/master/write-ups/synthesis_paper/metalab_synthesis.Rmd), and thus all analyses presented in the paper can be reproduced from this document.  
...
Below we compare the test of right-skew presented in the main text (Stouffer method) for both the full set of p-values and those only derived from test statistics. In no case does the significance of the test differ between the two analyses.
```{r}
...
The plot below present model coefficients for method for datasets with more than one method.Coefficients are estimated from random-effects meta-analytic models.
```{r, fig.height = 6}
...
for (i in 1:length(datasets$name)) {
    ...
    if (!(datasets$name[i] %in% single_method_datasets)) {
        ...
        model = rma(d_calc ~ method - 1, vi = d_var_calc, data = d, method = "REML")
...
method.betas$method = gsub("method", "", method.betas$method)

ggplot(method.betas, aes(x = dataset, y = betas, ymin = ci.lb, ymax = ci.ub, color = method)) +
