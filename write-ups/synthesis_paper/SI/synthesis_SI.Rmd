...
```{r}
atypical = all_data %>%  
  group_by(short_name, infant_type) %>%
  summarize(n = n())  %>%
  filter(infant_type  != "typical") %>%
  ungroup() %>%
  summarize(total_atypical = sum(n)) %>%
  as.numeric()

total = sum(studies$num_experiments)
```
In the main text, we analyzed effect sizes estimate from all populations, including typical and atypically developing children and mono- and multilingual children. It is possible, however, that children drawn from atypically developing or multilingual populations estimate different effect sizes than from their typically developing or monolingual counterparts. Here we re-analyze the dataset excluding conditions with atypically developing or multilingual children. These criteria excluded `r round(atypical/total, 2) * 100`% conditions, leaving `r total-atypical` total effect sizes.

Below, we reproduce the same set of replicability analyses as in Table 2 in the main text. Overall, excluding these conditions has little impact on the replicability analyses.
```{r}
all_data_typical = all_data %>%
  filter(infant_type == "typical")

all_ds_typical = all_data_typical %>%
  split(.$short_name) %>%
  map(function(ma_data) overall_es(ma_data,0)) %>%
  bind_rows() %>%
  mutate_each_(funs(round(., digits = 2)), 
...
fsn.package.data_typical = all_data_typical %>%
  group_by(dataset) %>%
  summarise(fsn_package = metafor::fsn(d_calc, d_var_calc, 
                         target = .01, type="Orwin")$fsnum) %>%
  left_join(datasets %>% select(name, short_name), by= c("dataset" = "name")) %>%
  rename(fsn_string = fsn_package)

funnel.es.data_typical = all_data_typical %>%
  mutate(dataset = as.factor(dataset),
         dataset= gdata::reorder.factor(dataset, 
                                    new.order = c(2,6,10,11,9,12,4,8,3,5,1,7)),
         dataset = plyr::revalue(dataset, 
...
eggers.data_typical = all_data_typical %>%
  ungroup() %>%
  split(.$short_name) %>%
  map(function(ma_data) eggers_tests(ma_data)) %>%
  bind_rows() %>%
...
pc.data <- get_all_pc_data(all_data_typical, ALPHA, P_INCREMENT, transform = TRUE) 
...
table.data.typical = left_join(select(all_ds_typical, c(short_name, d_string)), fsn.package.data_typical) %>%
             left_join(eggers.data_typical) %>%
             left_join(stouffer.data_typical) %>%
...
names(table.data.typical) = c("Phenomenon", "d", "Fail-Safe N", "Funnel Skew", "P-curve Skew")
kable(table.data.typical, booktabs = TRUE) 
```
