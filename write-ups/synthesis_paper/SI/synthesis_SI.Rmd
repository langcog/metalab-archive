...
opts_chunk$set(message = F, warning = F, 
               error = F, cache = F, tidy = T, tidy.opts=list(width.cutoff = 50))
```
...
```
Note that for Label Advantage effect sizes are calculated with chance (.5) as baseline, since the nature of the "control" conditions vary dramatically across experiments.
...
for (i in 1:length(datasets$name)) {
    ...
    if (!(datasets$name[i] %in% single_method_datasets)) {
        ...
        model <- metafor::rma.mv(d_calc ~ method - 1, V = d_var_calc, random = ~ 1 | short_cite, data = d)
