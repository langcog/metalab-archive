...
We next compare the test of right-skew presented in the main text (Stouffer method) for both the full set of p-values and those only derived from test statistics. In no case does the significance of the test differ between the two analyses.
```{r}
...
ggplot(method.betas, aes(x = dataset, y = betas, ymin = ci.lb, 
  ...
  #geom_line(aes(group = method, color = method), position=position_jitter(0.8))  +
