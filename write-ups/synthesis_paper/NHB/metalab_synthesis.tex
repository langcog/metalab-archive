...
e.g., \textbf{???} for review), only a small amount of work addresses
the question of relationships between different skills (e.g.,
\textbf{???}, \textbf{???}, \textbf{???}). Yet if such relationships
...
experimental findings (\textbf{???}, \textbf{???}). The fundamental unit
of meta-analysis is the \emph{effect size}: a scale-free, quantitative
measure of \enquote{success} in a phenomenon. Importantly, an effect
size provides an estimate of the size of an effect, as well as a measure
of uncertainty around this point estimate. With this quantitative
measure, we can apply the same reasoning we use to aggregate noisy
measurements over participants in a single study: By assuming each
study, rather than participant, is sampled from a population, we can
appeal to a statistical framework to combine estimates of the effect
...
another (\textbf{???}, \textbf{???}, \textbf{???}). Failed replications
are difficult to interpret, however, because they may result from a wide
...
studies is often difficult (\textbf{???}, \textbf{???}). By aggregating
...
always require more power to detect (\textbf{???}), it may be quite
difficult to identify size from individual papers. By aggregating across
papers using meta-analytic methods, however, we may be better able to
detect these changes, leading to more precise description of the
empirical phenomena.
...
size, fail-safe-N (\textbf{???}), funnel plots, and p-curve
(\textbf{???}, \textbf{???}, \textbf{???}). These analytical approaches
each have limitations, but taken together, they provide converging
evidence about whether an effect is likely to exist, and the extent to
which publication bias and other questionable research practices are
present in the literature. Overall, we find most phenomena in the
language acquisition literature have evidential value, and can therefore
provide the basis for theoretical development. We also find evidence for
some bias, as well as evidence that two phenomena---phonotactic learning
and statistical sound learning---likely describe null or near-null
effects.
...
This is called the \enquote{fail-safe} number of studies (\textbf{???}).
...
bias (\textbf{???}). Importantly, if experimenters are exercising
...
the main hypothesis across a literature (\textbf{???}, \textbf{???},
\textbf{???}). Critically, if there is a robust effect in the
...
(e.g., .01) than large values (e.g., .04). An important property of this
analysis is that we should expect this skew independent of any true
heterogeneity in the data, such as age. Evidence that the curve is in
...
children to select a novel object, given a novel word; \textbf{???})
suggests a steep developmental trajectory of this skill. We then can use
these data to build quantitative models to understand how aspects of
experience (e.g., vocabulary development) or maturational constraints
may be related to this trajectory (e.g., \textbf{???}, \textbf{???}).
...
acquisition of sound categories (e.g., \textbf{???}), word boundaries
(e.g., \textbf{???}), grammatical categories (e.g., \textbf{???}), and
even word learning (e.g., \textbf{???}).
...
of lower information (``blessing of abstraction,'' \textbf{???}). There
is evidence for this proposal from work that suggests word learning
supports the acquisition of lower-level information like phonemes
(\textbf{???}). More broadly, there is evidence that higher-level skills
like word learning may be acquired relatively early in development,
likely before lower level skills have been mastered (e.g., \textbf{???},
\textbf{???}).
...
The data and code reported in this paper have been deposited in GiHhub,
...
nocite: \textbar{} (\textbf{???}) (\textbf{???}) (\textbf{???})
(\textbf{???}) (\textbf{???}) (\textbf{???}) (\textbf{???})
(\textbf{???}) (\textbf{???}) (\textbf{???}) (\textbf{???}) \ldots{}
