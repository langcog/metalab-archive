...
p.source = pc.data %>%
  select(f.transform, f.value,dataset, study_ID) %>%
  group_by(dataset) %>%
  summarise(n.total = n(),
...
pc.data %>%
  ...
  geom_line(size = 1, aes(linetype = measure, color = measure)) +
  ...
  geom_text(aes(label = prop.ts.string, x = .045, y = .8), data = p.source, colour = "black") +
  theme_bw() + 
