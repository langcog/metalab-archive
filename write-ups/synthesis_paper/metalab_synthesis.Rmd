...
pc.data %>%
  ...
  geom_ribbon(aes(ymin = ci.lower, ymax = ci.upper, x = p), fill = "grey70", data = ci.data) +
  ...
  geom_text(aes(label = paste("num. total = ", n.total), x = .035, y = .87), data = p.source, 
            colour = "black", size = 2) +
  geom_text(aes(label = paste("prop. test stat. = ", prop.ts.string), x = .035, y = .8), data = p.source, colour = "black", size = 2) +
  geom_text(aes(label = paste("num. sig. ps = ", sig.p), x = .035, y = .73), data = p.source, colour = "black", size = 2) +
  theme_bw() + 
...
# paste below (so can edit latex - remove \addlinespace, fix header and row formatting):
...
Figure 2 shows p-curves for each of our `r nrow(datasets)` meta-analyses. All p-curves show evidence of right skew, with the exception of phonotactic learning and statistical sound learning (Table 2, column 5).
...
# need to use model fits rather than d_calc values

#predict(formula = ggrepel$d_calc~log(ggrepel$age.years), #ggrepel.data$age.years)
#predict(loess(dist~speed,cars), cars$speed)

#model <- loess(wt ~ hp, data=mtcars)
#model <-  y ~ log(x)
#pred <- predict(model, newdata = ggrepel.data$age.years, se=TRUE)
...
TO DO:
  ...
  - Write SI [description of systematicity + age vs. method on ES +  coded papers reporting experimental data]
  - write abstract
