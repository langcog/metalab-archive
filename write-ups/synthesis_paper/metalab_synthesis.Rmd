...
```{r, fig.pos = 'T',  fig.width=8, fig.height=4, fig.cap = "Effect size plotted as a function of age across all developmental meta-analyses in our dataset. Lines show logarithmic model fits. Each point corresponds to a condition, with the size of the point indicating the number of participants. [fix labels manually]"}
...
# Fit models using lmer
lin_lmer <- lmer(d_calc ~ mean_age + (log(mean_age)|dataset), 
...
p_predict_fits = ggplot(left_join(all_data_preds, ld.df),
...
ggrepel.data = simulated.development.all %>% 
  ...
  group_by(dataset) %>% 
  filter(d_calc > -.3 & d_calc < 2) %>%
  filter(age.years > 0 & age.years < 3.1) %>%
  slice(1) %>%
  filter(d_calc == max(d_calc)) 
...
ggplot(simulated.development.all,
       aes(x = age.years, y = d_calc, col = dataset, weight=1/n)) + 
...
#### Acknowledgments
We would like to thank Kyle MacDonald and Bria Long for their help with individual meta-analyses.
