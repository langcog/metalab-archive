...
# Introduction
To learn to speak a language, a child must acquire a wide range knowledge and skills: the sounds of the language, the word forms, and how the word forms map onto meanings, to name only a few. How does this process unfold? Our goal as psychologists is to build a theory that can explain and predict this process in a way that is both precise but also highly generalizable. The challenge we face, however is that we must build this theory on the basis of very limited data. And, as a consequence, we must rely on inductive reasoning strategies to go from the data we observe to the theories we posit. In this paper, we demonstrate how meta-analytic methods can support the inductive process of theory building in the case of language acquistion.

Meta-analysis is a quantiatiave method for aggregating across experimental findings. The fundamental unit of meta-analysis is the *effect size*: a scale-free, quantitative measure of "success" in a phenomenon. Importantly, an effect size provides an estimate of the *size* of an effect, as well as a measure of uncertainty around this point estimate. With  such a quantitative measure of success, we can apply the same reasoning we use to aggregate  noisy measurements over  participants in a single study: By assuming each *study*, rather than participant, is sampled from a population, we can appeal to the classical statistical framework to combine estimates of the effect size for a given phenomenon.

Meta-analytic methods support theory building in several ways. First, they provide a way of evaluating which effects in a literature are "real," and thus should constrain the theory. The field of psychology has grown concerned recently in the face of high-profile evidence that an effect observed in one study may not be the same in another ("replicability crisis"; Ioannidis, 2005; Nosek, 2012, 2015). At the stage of experimental planning, researchers must pre-specify analytical descision to limit ``researcher'' degrees of freedom (Simmons, 2011; Simonsohn, 2014a, 2014b, 2014c). At the stage of completion, researchers should share a result regardless of its significance (Rosenthal, 1979; Fanelli 2012). 

Importantly, however, some variability across studies is to be expected due to sampling error alone. Effect sizes and meta-analytic methods provide a principled approach for distinguising variability due to sampling error from variability in the underyling effect. With a more veridical description of the empirical landscape, we are in a better position to build a theory.

Second, meta-analysis support theory building by providing higher fidelity descriptions of phenomenona. Rather than simply concluding that an effect exists, effect sizes allow us  to describe as, for exapmple, How does the effect change over development? How does the effect compare in magnitude to other effects? How much does a moderator affect the effect? This type of continuous analysis supports building quantitative models, and specifying theories that are more precise and constraining.

(SYNTHESIS POINT) Furthermore, to build parsimonious theories of human behavior, we should seek to explain not just individual phenemenon, but entire literatures of research. What is needed, then, is a tool for aggregating noisy data across studies within a phenomenon, as well as a common language for comparing effects across phenomenona.
...
While meta-analytic methods are likely helpful for many psychological literatures, we believe language acquisition is a particularly informative application for this tool. One reason is that language acquistion may be uniquely vulnerable to false findings because running children is expensive, and thus sample sizes are small and studies are underpowered (Ioanndis, 2005). In addition, the difficulty in running participants means that replications are relatively rare in the field. Finally, there has been recent attention to developmental research practices more broadly, and corn [@Peterson:2016]. 

Towards this end, we have developed a dataset of effect sizes in the language acquistion literature, across 12 phenomenona, and XXX studies.   [Metalab](http://metalab.stanford.edu/). In what follows, we use this dataset to describe the e use this dataset below to first evaluate 
We have two goals:
...
Fidelity
* across development
* across different theoretical moderators (does X matter?)
* across phenomenoa 
...
This problem of induction is particularly challenging in the case of behaviors that rely on many different skills, such as langauge acquisition. To acquire a language





In achieving this goal, however, we are fundamentally challeged  e challenge we face, however, is that we must build these theories on the basis of limited data, typically collected from only 20-30 participants. This means that we must infer com

The developmental trajectory of language aquistion is 


Psychologists hope to build  generalizable theories about human behavior---theories that hold true beyond particulars of an individual study. 

This quantitative approach provides a rich tool kit for synthesizing across literatures. By describing different phenomena using the same unit of measurement, we are able to compare effects in different domains. 



In practice, however, the feasability of this meta-analytic approach relies on the field’s commitment to practices that facilitate cumulative science. These practices apply to all stages of the research process. And, at the stage of sharing, researchers must provide enough information about the method for another lab to conduct a close replication. Critically, sreports must also contain complete descriptions of both data and analytical decisions so that effect sizes can be calculcated for the purposes of meta-analysis,
...
One limitation of this analysis, however, is that it assumes that all reported effect sizes are obtained in the absence of analytical flexibility: If experimenters are exercising analytical flexibility through practices like p-hacking, then then number and magnitude of observed true effects in the literature may be inflated. In the next analysis, we examine this possibility through funnel plots.
...
P-curves provide a more robust way to identify bias in a literature [@simonsohn2014p; @simonsohn2014power; @simonsohn2015better]. A p-curve is the distribution of p-values for the statistical test of the main hypothesis across a literature. Critically, if there is a real effect in the literature, the shape of the p-curve should reflect this. In particular, we should expect the p-curve to be right skewed with more small values (e.g., .01) than large values (e.g., .04). An important features of this method is that we should expect this skew independent of any true heterogenity in the data, such as age. Evidence that the curve is in fact right-skewed would suggest that the literature is not biased, and that it provides evidential value for theory building. 
...
Given this literature has evidential value, we next turn to drawing inferences on the basis of the literature. 
