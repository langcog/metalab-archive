...
ci.data = pc.data %>%
  split(.$dataset) %>%
  map(function(data) get.all.CIS(data)) %>%
  bind_rows() %>%
  mutate(dataset = as.factor(dataset),
         dataset = plyr::revalue(dataset, 
                                 c("Infant directed speech preference"="IDS preference",
