...
simulated.development.all = mutate(simulated.development, dataset = as.factor(dataset)) %>%
  rbind(all_data.f) %>%
  mutate(model = ordered(model, levels = c("Sequential", "Simultaneous", "Other", "Observed")))
...
#   - what to do with control conditions?
        # filter for for label advantage
        # pair for alex's
#   - fix plot labels in adobe
#   - add pointing to meta-meta? looks kind of wonky.
#   
#   Writing:
