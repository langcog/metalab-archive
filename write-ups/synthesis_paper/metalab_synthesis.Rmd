...
eggers_tests <- function(ma_data){
    # model
    model.mod = rma(ma_data$d_calc ~ma_data$mean_age_1, ma_data$d_var_calc,
                             method = "REML",
                control = list(maxiter = 1000, stepadj = 0.5))
...
simulated.development = data.frame(age.years = c(x,x,x,x,x,x,x,x,x),
                                   ...
                                   model = rep(c("Sequential", "Simultaneous", "Other"), each=length(x)*3),
                                   n = 1)
...
#   - filter non-sigificant for meta-meta
#   
#   Writing:
...
# next: alex's stuff -> deal with figures.
```
