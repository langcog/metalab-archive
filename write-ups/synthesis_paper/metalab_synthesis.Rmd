...
# Introduction
To learn to speak a language, a child must acquire a wide range of knowledge and skills: the sounds of the language, the word forms, and how words map to meanings, to name only a few. How does this process unfold? Our goal as psychologists is to build a theory that can explain and predict this process. But, acquiring a language requires not just learning these skills in isolation; acquiring a language is the result of the integration of a range of skills across the language hierarchy: If the child is unable to segment "dog" from natural speech, a child cannot be expected to understand the meaning of this word. In building a theory of this system, a pragmatic research strategy has been to study these skills primarily in isolation, describing the developmental trajectory of individual phenomena in separate research programs. However, if it is indeed the case that there are dependencies between these skills, then we may not be able to ultimately understand one skill without having a more complete picture of other skills.

A substantial challenge in building this theory is that we must do so on the basis of limited, noisy experimental findings. As a consequence, the raw material of our theories are often contradictory. This leaves the theorist with uncertainty about which experimental findings should constrain the theory. Here, we suggest that reframing experimental findings in terms of quantitative, rather than qualitative, descriptions, we can aggregate across findings to evaluate the evidential strength for a phenomenon. Furthemore, by treating the raw data of our theories as quantitative estimates, we can make quantitative predictions, and build more constraining theories. In this paper, we consider the domain of language acquisition and demonstrate how meta-analytic methods can support these two goals. 
...
To assess the replicability of language acquisition phenomena, we conducted several diagnostic analyses: Meta-analytic estimates of effect size, fail-safe-N [@orwin1983fail], funnel plots, and p-curve [@simonsohn2014p; @simonsohn2014power; @simonsohn2015better]. These analytical approaches each have limitations, but taken together, they provide converging evidence about the presence of publication bias, questionable research practices, and  the replicability of a literature. Overall, we find most phenomena in the language acquistion literature has evidential value, and should therefore provide the basis for theoretical development.  evidence of bias in our meta-analyses, suggesting that the language acquisition literature likely describes real psychological phenomena and  [FIX THIS]
...
```{r, fig.pos = 'T',  fig.width=8, fig.height=4, fig.cap = "Effect size plotted as a function of age across all developmental meta-analyses in our dataset. Lines show logarithmic model fits. Each point corresponds to a condition, with the size of the point indicating the number of participants."}
...
```{r,  fig.width=8, fig.height=4, fig.cap="Effect size as a function of age from 0-3 years, across 11 different phenomena. These developmental curves suggest there is interactivity across language skills, rather than sequential learning of the linguistic hierarchy", fig.height=4, fig.width=8, fig.pos = "T"}
# real data
...
simulated.development = data.frame(age.years = c(x,x,x,x,x,x,x,x,x),
                                   ...
                                   model = rep(c("bottom-up", "synergistic", "ad hoc"), each=length(x)*3),
                                   n = NA,
                                   d_var_calc = NA)
...
simulated.development.all$model <- ordered(simulated.development.all$model, 
                                            levels = c("bottom-up", "synergistic", "ad hoc", "observed"))
