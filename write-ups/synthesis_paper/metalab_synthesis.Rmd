...
source("/Documents/GRADUATE_SCHOOL/Projects/metalab/metalab/dashboard/global.R",chdir = T)
...
Estimates of effect size were based on journal reports of experimental data.  In total, our sample includes estimates from `r sum(datasets$num_papers)` papers, `r sum(datasets$num_experiments)` different experiments and `r format(floor(sum(datasets$num_subject)), big.mark=",", scientific=FALSE)` participants. 
