...
funnel95 <- data.frame(funnel95.x = c(center - lower_lim * 1.96, center,
                             center + lower_lim * 1.96),
                       funnel95yx = c(-lower_lim, 0, -lower_lim))

funnel99 <- data.frame(x = c(center - lower_lim * EXPAND_FACTOR, center,
                             center + lower_lim * EXPAND_FACTOR),
                       y = c(-lower_lim, 0, -lower_lim))

ggplot(funnel.es.data, aes(x = es, y = -se)) +
  scale_colour_solarized(name = "") +
  #scale_x_continuous(limits = c(left_lim99[1], right_lim99[1])) +
  scale_y_continuous(expand = c(0, 0),
                     breaks = round(seq(0, -max(funnel.es.data$se), length.out = 5), 2),
                     labels = round(seq(0, max(funnel.es.data$se), length.out = 5), 2)) +
