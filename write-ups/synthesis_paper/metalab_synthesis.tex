...
Meta-Analysis}
  ...
  \affiliation{
    \vspace{0.5cm}
          ...
          \textsuperscript{4} NeuroPsychologie Interventionnelle (NPI), ENS  }


%   \def\affinst{{"init", "Department of Psychology, Stanford University", "Department of Brain and Cognitive Sciences, MIT", "Laboratoire de Sciences Cognitives et Psycholinguistique, ENS", "NeuroPsychologie Interventionnelle (NPI), ENS"}}%
%   \def\affstate{{"init", "", "", "", ""}}%
%   \def\affcntry{{"init", "", "", "", ""}}%
...
because running children is expensive, and thus sample sizes are often
small and studies are underpowered. In addition, the high cost and
practical difficulties associated with collecting large developmental
datasets means that replications are relatively rare in the field.
Meta-analysis provides a method for addressing these issues by
harnessing existing data to estimate effect sizes and developmental
trends.
...
\href{http://metalab.stanford.edu}{http://metalab.stanford.edu/}). We
demonstrate how meta-analysis supports building this theory in two ways.
We first use meta-analytic techniques to evaluate the evidential value
of the empirical landscape in language acquisition research. We find
broadly that this literature has strong evidential value, and thus that
the effects reported in the literature should constrain our theorizing
of language acquisition. We then turn toward the task of synthesizing
these findings across phenomena and offer a preliminary, quantitative
...
evidential value, and can therefore provide the basis for theoretical
...
indicating that these literatures do not describe robust effects
(Cristia, in prep.).
...
for each dataset that are less than .05 and thus represented in each
...
aquisition. In future research, we can use this approach to distinguish
...
controling for other possible differences between experiments. Others
...
non-native vowel discrimintation, while the current analysis suggests a
