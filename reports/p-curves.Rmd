...
obs_es =  all_data %>%
  group_by(dataset) %>%
  summarise(d_obs = summary(rma(d_calc,d_var_calc,
             slab = short_cite, 
             data = .))$b[1],
            ci_lb_obs = summary(rma(d_calc,d_var_calc,
             slab = short_cite, 
             data = .))$ci.lb[1],
            ci_ub_obs = summary(rma(d_calc,d_var_calc,
             slab = short_cite, 
             data = .))$ci.ub[1]) 

all_es = left_join(obs_es, pc_es) %>%
  gather("d", "val", c(2,5,8)) %>%
   #gather("d2", "val2", 3:5) %>%
  as.data.frame()
