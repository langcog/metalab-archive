...
eggers_tests_age <- function(ma_data){
    model = rma(d_calc~ mean_age, d_var_calc, data = ma_data) # model
    egg.random = regtest(model) # Egger's test
    data.frame(dataset = ma_data$short_name[1],
               egg.random.z.age = egg.random$zval,
               egg.random.p.age = egg.random$pval)
}

eggers.data.age = all_data %>%
  ungroup() %>%
  split(.$short_name) %>%
  map(function(ma_data) eggers_tests_age(ma_data)) %>%
  bind_rows()
```
...
table.data = all_ds %>%
             ...
             left_join(eggers.data.age, by = c("short_name" = "dataset"))

             #left_join(stouffer.data) %>%
             #select(-dplyr::contains("dataset"), -stouffer.sig)

write.csv(table.data, "ES_data_for_networks2.csv")
```
