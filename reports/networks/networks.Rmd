...
getModularity <- function (my.paper.data, 
    ...
    if (length(this.net > 0)) {
      ...
      clustering = cluster_leading_eigen(graph, options = list(maxiter = 1000000000))
...
ma.es = read.csv("ES_data_for_networks2.csv") %>%
  #select(short_name, overall.d, overall.d.age,
  #       fsn_string, egg.random.z, d.age, tau2, I2, H2) %>% 
...
getModularity_paper <- function (my.paper.data, 
    ...
    if (length(this.net > 0)) {
      ...
      data.frame(analysis = this.analysis,
                 network = this.network,
                 closeness = estimate_closeness(graph, cutoff = 100, normalized = TRUE),
                 betweeness = estimate_betweenness(graph, cutoff = 100, normalized = TRUE),
                 degree = igraph::degree(graph, normalized = TRUE))
    }
}
...
```{r, eval = F}
A = read.csv("wos_bib1_6.csv") %>%
  mutate_all(as.character())
B = read.csv("wos_bib1_5.csv")%>%
  mutate_all(as.character())

A == B

d = mapply(function(x, y) all(x==y, na.rm = TRUE), A, B)
